 ...
 *
 * GET /api/manifestValidate?manifest=<key> -> { wellFormed, violations[] } (or the honest
 * legacy verdict { wellFormed:false, reason:'legacy ...' }). Admin/super only. A
 * not-well-formed verdict is normal data, NOT an HTTP error.
 */
...
const moduleFunction = function ({
	...
	const serviceFunction = (permissionValidator) => (xReq, xRes, next) => {
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, result) => {
				if (err) {
					next(`Manifest validate failed: ${err}`, args);
					return;
				}
				next('', { ...args, result });
			};
			accessPointsDotD['manifest-validate'](requestData, localCallback);
		});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			if (err) {
				...
				xLog.error(`Manifest validate error (${errorId}): ${err}`);
	...
	const method = 'get';
	const thisEndpointName = 'manifestValidate';
