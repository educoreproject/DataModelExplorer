...
/**
 * ENDPOINT: MANIFEST ASSIGN (add a collaborator)
 *
 * POST /api/manifestAssign  body { manifestKey, userId } -> { assigned:true, ... }.
 * Admin/super only. Idempotent; mutable side-metadata (never mints a new manifest).
 */
...
const moduleFunction = function ({
	...
	const serviceFunction = (permissionValidator) => (xReq, xRes, next) => {
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, result) => {
				if (err) {
					next(`Manifest assign failed: ${err}`, args);
					return;
				}
				next('', { ...args, result });
			};
			accessPointsDotD['manifest-assign'](requestData, localCallback);
		});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			if (err) {
				...
				xLog.error(`Manifest assign error (${errorId}): ${err}`);
	...
	const method = 'post';
	const thisEndpointName = 'manifestAssign';
