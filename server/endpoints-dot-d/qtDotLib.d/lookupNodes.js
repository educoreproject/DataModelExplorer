...
const moduleFunction = function ({
	...
	const serviceFunction = (permissionValidator) => (xReq, xRes, next) => {
		...
		// --------------------------------------------------------------------------------
		// STEP 2: CALL GET-NODES ACCESS POINT
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, result) => {
				if (err) {
					next(`lookupNodes query failed: ${err}`, args);
					return;
				}
				next('', { ...args, result });
			};

			accessPointsDotD['get-nodes'](xQuery, localCallback);
		});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			if (err) {
				...
				xLog.error(`lookupNodes error (${errorId}): ${err}`);
