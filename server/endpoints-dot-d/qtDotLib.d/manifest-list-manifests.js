...
/**
 * ENDPOINT: MANIFEST LIST MANIFESTS
 *
 * GET /api/manifestListManifests -- the manifest list for the Manifest Editor.
 * Admin/super only. Delegates to the manifest-list-manifests access point, which
 * shells out to educoreForge's `manifestEditor -listManifests`.
 */
...
const moduleFunction = function ({
	...
	const serviceFunction = (permissionValidator) => (xReq, xRes, next) => {
		...
		// --------------------------------------------------------------------------------
		// STEP 2: LIST MANIFESTS VIA ACCESS POINT
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, result) => {
				if (err) {
					next(`Manifest listing failed: ${err}`, args);
					return;
				}
				next('', { ...args, result });
			};

			accessPointsDotD['manifest-list-manifests']({}, localCallback);
		});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			if (err) {
				...
				xLog.error(`Manifest listing error (${errorId}): ${err}`);
	...
	const method = 'get';
	const thisEndpointName = 'manifestListManifests';
