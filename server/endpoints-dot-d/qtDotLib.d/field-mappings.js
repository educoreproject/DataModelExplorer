...
const moduleFunction = function ({
	...
	const serviceFunction = (permissionValidator) => (xReq, xRes, next) => {
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, result) => {
				if (err) {
					next(`field-mappings query failed: ${err}`, args);
					return;
				}
				next('', { ...args, result });
			};
			accessPointsDotD['field-mappings'](xQuery, localCallback);
		});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			if (err) {
				...
				xLog.error(`field-mappings error (${errorId}): ${err}`);
	...
	const method = 'get';
	const thisEndpointName = 'fieldMappings';
