		...
		const defaultAdd = (item, callback) => {
			...
			try {
				...
				item.exceptionList.push(
					`ignoring invalid file: ${path.basename(item.endpointPath)}: ${err.toString().split('\n').qtFirst('n/a')}`,
				);
			}
		...
		const toString = () => {
			const formatProperty = (item) => {
				...
				if (typeof item == 'function') {
					...
					const args = result.qtGetSurePath('groups.args', 'function');
					outString = `(${args == ')' ? args + ' # function takes no arguments' : args}`;
				}
				return outString;
			};
