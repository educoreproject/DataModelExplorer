...
const singleCall = async ({ prompt, systemPrompt, sessionContext, config, timing, onEvent }) => {
	...
	tm.mark('singleCall-direct', 'prompt_build_start');
	let userMessage = prompt;
	if (sessionContext) {
		userMessage = `${sessionContext}\n\n--- NEW PROMPT ---\n${prompt}`;
	}
	tm.mark('singleCall-direct', 'prompt_build_done', {
	...
	emitEvent({ type: 'done' });
	tm.mark('singleCall-direct', 'stream_done', {
