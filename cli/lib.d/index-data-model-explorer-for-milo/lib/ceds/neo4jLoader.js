...
// Derived from index-ontology14-for-milo/lib/neo4jLoader.js
// Key difference: clears only :CEDS nodes (not full graph) for per-standard reload
...
const loadToNeo4j = async ({ entities, version, counts, neo4jBoltUri, neo4jUser, neo4jPassword }) => {
	...
	xLog.status(`[ceds/neo4jLoader] Connecting to Neo4j at ${neo4jBoltUri}...`);
	const driver = neo4j.driver(neo4jBoltUri, neo4j.auth.basic(neo4jUser, neo4jPassword), { encrypted: false });

	try {
		await driver.verifyConnectivity();
		xLog.status('[ceds/neo4jLoader] Connected to Neo4j.');
	} catch (err) {
	...
	try {
		// Clear only CEDS nodes (preserve SIF and bridge data)
		xLog.status('[ceds/neo4jLoader] Clearing existing CEDS nodes...');
		await session.run('MATCH (n:CEDS) DETACH DELETE n');

		// Constraints
		xLog.status('[ceds/neo4jLoader] Creating constraints...');
		const constraints = [
		...
		// Indexes on uri
		xLog.status('[ceds/neo4jLoader] Creating indexes...');
		const indexes = [
		...
		if (skipped > 0) {
			xLog.status(`[ceds/neo4jLoader] Skipped ${skipped} entities with null cedsId`);
		}

		// Load CedsClass nodes
		xLog.status(`[ceds/neo4jLoader] Loading ${classes.length} CedsClass nodes...`);
		...
		xLog.status(`[ceds/neo4jLoader] Loading ${properties.length} CedsProperty nodes...`);
		await runBatched(session, `
			UNWIND $batch AS item
		...
		xLog.status(`[ceds/neo4jLoader] Loading ${optionSets.length} CedsOptionSet nodes...`);
		await runBatched(session, `
			UNWIND $batch AS item
		...
		xLog.status(`[ceds/neo4jLoader] Loading ${optionValues.length} CedsOptionValue nodes...`);
		await runBatched(session, `
			UNWIND $batch AS item
		...
		xLog.status(`[ceds/neo4jLoader] Creating ${hasPropertyBatch.length} HAS_PROPERTY relationships...`);
		await runBatched(session, `
			UNWIND $batch AS item
		...
		xLog.status(`[ceds/neo4jLoader] Creating ${hasOptionSetBatch.length} HAS_OPTION_SET relationships...`);
		await runBatched(session, `
			UNWIND $batch AS item
			MATCH (p:CedsProperty:CEDS {cedsId: item.propCedsId})
		...
		xLog.status(`[ceds/neo4jLoader] Creating ${hasValueBatch.length} HAS_VALUE relationships...`);
		await runBatched(session, `
			UNWIND $batch AS item
		...
		xLog.status(`[ceds/neo4jLoader] Creating ${subclassBatch.length} SUBCLASS_OF relationships...`);
		await runBatched(session, `
			UNWIND $batch AS item
		...
		// Root node
		xLog.status('[ceds/neo4jLoader] Creating CedsOntology root node...');
		await session.run(`
		...
		xLog.status('[ceds/neo4jLoader] Creating PART_OF relationships to root...');
		await session.run(`
		...
		xLog.status('[ceds/neo4jLoader] === CEDS LOAD COMPLETE ===');
