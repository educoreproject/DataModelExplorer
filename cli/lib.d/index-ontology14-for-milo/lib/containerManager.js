...
// containerManager.js — Docker lifecycle management for CEDS Ontology Neo4j containers
// Adapted from index-text-graph-for-milo/lib/containerManager.js for structured graph tools.
// Key differences: no rag_ prefix, no Chunk index, writes search .ini for ontology search tools,
// detects externally-configured databases via existing search .ini.
...
const startContainer = ({ voyageApiKey, providerProjectRoot, providerDir, assetsDir }, callback) => {
	...
	if (fs.existsSync(instanceConfigPath)) {
		...
		const ensureSearchIni = () => {
			generateSearchIni({
				...
				searchModuleName: neo4jConfig.searchModuleName || 'cedsOntology14Search',
			});
		};
	...
	// (e.g., manually created container on dev machine)
	// -----------------------------------------------------------------
	...
	const searchModuleName = (assetManifest && assetManifest.searchModuleName) || 'cedsOntology14Search';
	...
	findAvailablePortPair(7700, (err, ports) => {
		...
		const substitutions = {
			...
			searchModuleName: assetManifest.searchModuleName || 'cedsOntology14Search',
		};
		...
		const authEnv = assetManifest ? '' : `-e NEO4J_AUTH=neo4j/${generatedPassword}`;
		const dockerCmd = `docker run -d --name ${containerName} --restart unless-stopped -p ${boltPort}:7687 -p ${httpPort}:7474 ${authEnv} -e NEO4J_PLUGINS='["apoc"]' -e NEO4J_dbms_security_procedures_unrestricted=apoc.* -e NEO4J_dbms_security_procedures_allowlist=apoc.* -v ${dataDir}/data:/data -v ${dataDir}/logs:/logs -v ${dataDir}/plugins:/plugins -v ${dataDir}/import:/var/lib/neo4j/import neo4j:5.26`;
...
const generateSearchIni = ({ providerProjectRoot, providerName, boltPort, neo4jPassword, voyageApiKey, searchModuleName }) => {
	const { xLog } = process.global;
...
const statusContainer = ({ providerProjectRoot }, callback) => {
	...
	if (running) {
		...
		const countCmd = `docker exec ${containerName} cypher-shell -u neo4j -p ${password} -a bolt://localhost:7687 "MATCH (n:CEDS) RETURN count(n) AS total"`;
		exec(countCmd, (err, stdout) => {
			if (!err && stdout) {
				const match = stdout.match(/(\d+)/);
				if (match) {
					xLog.status(`[containerManager] CEDS node count: ${match[1]}`);
				}
			}
