// _useCaseData.js — inline snapshot of use case taxonomy + list.
// Extracted from the prior useCaseStore.js (pre-liveDataStores branch).
// DEVIATION FROM SPEC §7 Phase D: spec calls for /api/useCases driving the
// useCases slice from the DME graph. The graph's UseCase nodes lack the
// slug ids and subcategory mapping the UI currently relies on, so for Phase D
// we preserve the static data inline and wire the placeholder plumbing
// (loadUseCases / invalidateUseCases). Lifting this to the live graph is an
// open item — see LOG-liveDataStores-implementation.md Phase D notes.
