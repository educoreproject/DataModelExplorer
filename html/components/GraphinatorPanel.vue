...
const props = defineProps({
	...
	// Example prompts shown in the prompts dialog. Each item may be a string
	// or an object with { text }. Empty array hides the dialog entirely.
	examplePrompts: {
		type: Array,
		default: () => [],
	},

	// Optional welcome HTML rendered as a fallback when no <slot name="welcome">
	// is provided by the consuming wrapper page.
	welcomeText: {
		type: String,
		default: '',
	},
...
// Normalize prop into array of strings; supports string[] or [{text}].
				...
				<v-card-text style="max-height: 400px; padding: 0;">
					...
					<v-list v-if="graphStore.sessionList.length > 0" density="compact">
						...
						>
							...
							<v-list-item-subtitle>{{ formatSessionDate(session.createdAt) }}</v-list-item-subtitle>
						</v-list-item>
					</v-list>
