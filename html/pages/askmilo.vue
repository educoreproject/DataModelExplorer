...
import { useAskMiloStore } from '@/stores/askMiloStore';
...
const askMiloTabs = [
	{ label: 'Dialog', value: 'dialog', icon: 'mdi-chat' },
];
...
// availableTools comes from the server via WebSocket config channel (askMiloStore.availableTools)
...
<template>
	<v-app>
		...
		<v-main style="padding-top: 65px">
			<SubPageNav v-model="activeTab" :tabs="askMiloTabs" />
			...
			<v-container v-show="activeTab === 'dialog'" fluid class="pa-4 askmilo-container">
					...
					<div class="input-controls">
						...
						<v-card min-width="340" max-width="420" class="settings-card">
							...
							<v-card-text class="pt-2">
								<!-- Basic controls -->
								<v-select
									v-model="askMiloStore.settings.model"
									:items="modelOptions"
									label="Model"
									density="compact"
									variant="outlined"
							...
							<v-btn
								color="primary"
								@click="submitPrompt"
								:loading="askMiloStore.loading"
								:disabled="!askMiloStore.connected || !promptText.trim()"
								size="large"
				...
				<!-- Status alert -->
				<v-alert
					v-if="askMiloStore.statusMsg"
					type="warning"
					density="compact"
					class="mt-2"
					closable
					@click:close="askMiloStore.statusMsg = ''"
				>
					{{ askMiloStore.statusMsg }}
				</v-alert>

				<!-- Connection indicator -->
				<div class="connection-status">
					<v-chip
						:color="askMiloStore.connected ? 'success' : 'error'"
						size="small"
						variant="flat"
					>
						{{ askMiloStore.connected ? 'Connected' : 'Disconnected' }}
					</v-chip>
				</div>
			</v-container>
		</v-main>
