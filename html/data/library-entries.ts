...
export const libraryEntries = [
  {
    ...
    description: 'IEEE 1484.2-2024 Recommended Practice for Learning and Employment Record (LER) Ecosystems helps system architects and engineers design and deploy systems that leverage shared standards and technologies to support ecosystem-scale issuing, holding, and presenting verifiable credentials for education, skills-based hiring, and career advancement. Published in 2024 by the IEEE Learning Technology Standards Committee (LTSC).',
    owner: 'IEEE Learning Technology Standards Committee (LTSC)',
    governanceBody: 'IEEE Learning Technology Standards Committee (LTSC)',
  ...
  // ─── CEDS ──────────────────────────────────────────────────────────────────
  {
    ...
    description: 'CEDS is a U.S. Department of Education-funded, community-driven initiative that provides a common vocabulary for education data — a "Rosetta Stone" with standardized definitions for over 1,500 data elements spanning early childhood through workforce (P-20W). CEDS does not prescribe how to transport data; it defines what the data means. The initiative publishes a Data Dictionary, a Logical Data Model, and a machine-readable CEDS Ontology (OWL) used by state longitudinal data systems (SLDS) and researchers comparing data across states, agencies, and contexts.',
  ...
  // ─── SIF ───────────────────────────────────────────────────────────────────
  {
    ...
    description: 'SIF is a data sharing specification for K-12 education systems maintained by the Access 4 Learning Community (A4L). SIF 3.7 defines a REST-based infrastructure for exchanging education data objects between systems using a publish/subscribe and request/response model, with a comprehensive data model covering student demographics, enrollment, scheduling, assessment, and more. A4L is evolving the infrastructure into the "Unity" specification (SIF/JEDx Infrastructure) for modern web-based RESTful API integrations.',
  ...
  // ─── Ed-Fi ─────────────────────────────────────────────────────────────────
  {
    ...
    description: 'The Ed-Fi Data Standard, from the Michael & Susan Dell Foundation-funded Ed-Fi Alliance, is a CEDS-aligned, REST API-based data model for K-12. Unlike message-only specs, Ed-Fi provides a standardized Operational Data Store (ODS) where districts land data from their SIS, assessment, and attendance systems in one unified format — giving schools a 360° view of a student and powering state-level reporting and real-time dashboards. The Ed-Fi ODS/API is the reference implementation used by state education agencies and districts nationwide.',
  ...
  // ─── LIF ───────────────────────────────────────────────────────────────────
  {
    ...
    fieldMappingDescription: 'JSON-based data model with 8 core entities: Person, Credential, Assessment, CompetencyFramework, Course, Program, Organization, Position. Each property carries a useRecommendations annotation (often privacy- or equity-relevant). LIF fields map to CEDS elements — e.g., Assessment.identifier → CEDS AssessmentIdentifier, Person.Name → CEDS PersonName, Credential.name → CEDS CredentialDefinition.',
    transformationNotes: 'LIF is designed as a comprehensive interoperability data model across education and workforce. Covers territory of multiple specs: Assessment (like CEDS Assessment domain), CompetencyFramework (like CASE), Credential (like OB3/CLR), Position (workforce). Every property carries a single useRecommendations text field that may flag equity or privacy concerns. Missing: no specific wire protocol defined (transport-agnostic). Unique: includes Person entity with consent-aware modeling and demographic sensitivity guidance.',
    description: 'The Learner Information Framework (LIF) 2.0 is a machine-readable JSON/OpenAPI schema covering the P-20W lifecycle. LIF advertises 8 core entities (Person, Credential, Assessment, CompetencyFramework, Course, Program, Organization, Position) and carries roughly 1,000 properties — ~260 directly attached to those entities and the rest nested in composite sub-structures. Every property includes a free-text useRecommendations annotation that surfaces privacy- and equity-relevant guidance where applicable (many are marked "No change"; some flag sensitive-identifier handling or assessment-bias concerns).',
    owner: 'LIF (Learner Information Framework)',
    governanceBody: 'LIF Work Group',
    ...
    authoritativeRepoUrl: null,
    tags: ['LIF', 'LER', 'P-20W', 'interoperability', 'equity', 'privacy-by-design', 'workforce', 'credentials', 'assessments', 'competencies', 'K-12', 'postsecondary', 'use-recommendations'],
    aiTaxonomy: ['comprehensive-data-model', 'LER-interoperability', 'equity-aware'],
    aiSummary: 'Comprehensive education-to-workforce data model with 8 entities and ~1,000 properties, each annotated with a useRecommendations field that can surface equity or privacy guidance. Use LIF when you need a single interoperability model spanning credentials, assessments, competencies, courses, programs, organizations, and employment positions with built-in sensitivity guidance.',
    aiUnlocksSummary: 'Provides a unified data model that bridges the gap between education-focused standards (CASE, OB3, CLR) and workforce systems. Unique in including Person-level consent considerations and useRecommendations annotations at the property level. Positions your platform to handle the full learner lifecycle from assessment through employment.',
    implementationBurden: 'medium',
    implementationBurdenRationale: 'LIF is a data model specification, not a wire protocol — implementation involves mapping your system\'s data to LIF entities and their properties. The model is broad (8 entities, ~1,000 properties including nested composites) but cleanly structured. JSON-based payloads. useRecommendations annotations require review during implementation.',
    burdenRubric: {
      engineering: { level: 'moderate', note: 'Mapping to 8 entities with ~1,000 properties (across top-level and nested composites) requires significant data modeling effort. JSON payloads are straightforward. useRecommendations annotations add review overhead.' },
      ...
      legal: { level: 'low', note: 'Open specification. No licensing fees. useRecommendations help with compliance review but do not replace legal review.' },
    },
  ...
  "Person": {
    ...
    knownAdopters: ['LIF Work Group participants', 'State longitudinal data system pilots'],
    technicalDocLinks: [
